"""
Taken from https://github.com/deep-security/smartcheck-samples
"""
...
class Smartcheck(_SlightlyImprovedSession):
    ...
    def __init__(self, base, user, password, verify=True, trace=False, **kwargs):
        ...
        #print(f"Smartcheck({base}, {user}, {password}, {verify})")
    ...
    def last_scan(self, image, **kwargs):
        """Get last image scan."""
        ...
        for scan in self._list('/api/scans', LastScanException, 'scans', params=params, **kwargs):
            return scan
    ...
    def list_registries(self, **kwargs):
        """List registries."""
        for scan in self._list('/api/registries', ListRegistriesException,
                               'registries', params={}, **kwargs):
            yield scan

    def list_images(self, id, **kwargs):
        """List images."""
        for scan in self._list(f'/api/registries/{id}/images', ListRegistriesException,
                               'images', **kwargs):
            yield scan
...
class ListRegistriesException(Exception):
    def __init__(self, kind, response):
        super(ListRegistriesException, self).__init__(
            f'*** WARNING: could not retrieve {kind}: {response}'
        )
...
class LastScanException(ListException):
    def __init__(self, response):
        super().__init__('scans', response)
